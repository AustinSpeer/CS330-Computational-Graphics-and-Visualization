...
{
	// This records where he mouse is when it is first moved, so that the camera does not jump when the mouse is first moved. 
	if (gFirstMouse)
	{
		gLastX = xMousePos;
		gLastY = yMousePos;
...
{
	gMovementSpeed += static_cast<float>(yOffset) * 0.8f;

	if (gMovementSpeed < 0.2f)
		gMovementSpeed = 0.2f;
...
void ViewManager::ProcessKeyboardEvents()
{
	...
	if (glfwGetKey(m_pWindow, GLFW_KEY_O) == GLFW_PRESS && !oWasPressed)
	{
	...
	if (glfwGetKey(m_pWindow, GLFW_KEY_P) == GLFW_PRESS && !pWasPressed)
	{
	...
	// process camera movving in and out
	...
	//process camera strafing left and right
